...
const UserListAddress = () => {
    ...
    const [addressesEnvio, setAddressesEnvio] = useState<IDireccion[]>([]);
    const [addressesFacturacion, setAddressesFacturacion] = useState<IDireccion[]>([]);
        ...
        const fetchAddress = async () => {
            try {
                ...
                addresses.forEach(item => {
                    if (item.tipoDireccion?.includes('Envio')) {
                        setAddressesEnvio([{ ...item }]);

                    }
                    else {
                        setAddressesFacturacion([{ ...item }]);
                    }
                });
                ...
                {addressesEnvio.map((addressesEnvio) => (
                    <Col key={addressesEnvio.idDireccion} lg={12} className='d-flex justify-content-start user-mod-dir-radio'>
                        <input
                            ...
                            onChange={() => handleAddressChange(addressesEnvio.idDireccion)}
                        />
                ...
                {addressesFacturacion?.map((addressesFacturacion) => (
                    <Col key={addressesFacturacion.idDireccion} lg={12} className='d-flex justify-content-start user-mod-dir-radio'>
                        <input
                            ...
                            onChange={() => handleAddressChange(addressesFacturacion.idDireccion)}
                        />
